...
- **Presentation.** Use terminal-default body text and a single blue accent; community and credential entries stack vertically with published links. `whoami` JSON includes the featured `experience` list; credential JSON groups `certifications`, `thesis`, and `specializations`. Keep text and JSON information consistent and document schema migrations.
